...
class Image(models.Model):
    ...
    Model representing an image associated with a specific destination.
        destination (ForeignKey): Foreign key to the Destination model,
        representing the destination this image is associated with.
        image (CloudinaryField): Field to store the image using Cloudinary.

        __str__(): Returns a string representation of the image, including its
        ID and the name of the associated destination.
    Note:
        This model allows multiple images to be associated with a single
        destination.
...
RATING_CHOICES = [(i, f"{i} Star") for i in range(0, 6)]
